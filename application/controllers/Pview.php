    ...
    public function sendToPool($name,$email,$phone,$utm_source,$utm_medium,$utm_campaign,$utm_term,$utm_content,$utm_site,$utm_url,$utm_title,$utm_timestamp,$utm_itemID)
	{
		...
		$url = 'http://www.poolcrm.co.in/addLeadAPI.php?api_username=totalenvironmentquietUser&api_password=totalenvironmentquietPassword&builder_id=00000039&project_id=00000113&first_name='.$name.'&phone='.$phone.'&email='.$email.'&utm_source='.$utm_source.'&utm_medium='.$utm_medium.'&utm_campaign='.$utm_campaign.'&utm_term='.$utm_term.'&utm_content='.$utm_content.'&utm_site='.$utm_site.'&utm_url='.$utm_url.'&utm_title='.$utm_title.'&utm_timestamp='.$utm_timestamp.'&utm_itemID='.$utm_itemID;
	...
	public function sendWhatsapp()
	{
		...
		if($this->User_Model->insertData($data))
		{
        	...
        	$resultData = $this->sendToPool($name,$email,$mobile,$utm_source,$utm_medium,$utm_campaign,$utm_term,
			$utm_content,$utm_site,$utm_url,$utm_title,$utm_timestamp,$utm_itemID);
...
// 			$lead_id = $this->User_Model->get_id($name, $mobile, $email, $employee_name);
			...
			$this->email->cc('vishwanath.rao@sanatibuilders.com,manjunath.rao@sanatibuilders.com,surendra.reddy@sanatibuilders.com,surendra.reddy@sanatibuilders.com,manojbiswal@orangedigitamedia.in');
			$this->email->bcc('vrao.dm@gmail.com');
			$subject = 'Contact Us | Total Environment'; 
			$message = 'Leads For Total Environment:
			...
			$this->email->to($to);
			$this->email->subject('Leads for Total Environment');
	...
	// 		}
			...
			{
		//	echo 'Email sent.';
				redirect('https://api.whatsapp.com/send?phone=+916360690190&text=Hi%20I%20am%20interested%20in%20Total%Environment%20Project%20,pls%20call%20back%20and%20share%20the%20project%20details.');
			}
			else
			{
